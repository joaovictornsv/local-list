...
export const useTaskStorage = () => {
  ...
  const importTasks = (newTasks) => {
    ...
    const sectionTasksNonConflicted = newSectionTasks
      .map((newSectionTask) => {
        const taskSectionExists = getTasksBySectionId(
          newSectionTask.sectionId,
        ).find((task) => task.title === newSectionTask.title);
    ...
    saveTasks([...tasks, ...newIndependentTasks, ...sectionTasksNonConflicted]);
  };

  const duplicateSectionTasks = ({ sectionId, newSectionId }) => {
    const sectionTasks = getTasksBySectionId(sectionId);

    const duplicatedSectionTasks = sectionTasks.map((task) => ({
      ...task,
      id: generateRandomUuid(),
      sectionId: newSectionId,
    }));
    saveTasks([...tasks, ...duplicatedSectionTasks]);
  };
